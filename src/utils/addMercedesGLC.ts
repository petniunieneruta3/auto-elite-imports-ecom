...
export const addMercedesGLC = async () => {
  try {
    ...
    // Now add all the images to the vehicle_images table
    const images = [
      {
    ...
    return { vehicle: vehicleData, success: true };

  } catch (error) {
    console.error('Error in addMercedesGLC:', error);
